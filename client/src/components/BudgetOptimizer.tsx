...
const BudgetOptimizer = () => {
  ...
  const [suggestions, setSuggestions] = useState<{ category: string; reduced_amount: number; new_amount: number }[]>(
    []
  );
  ...
  const getBudgetingSuggestions = async () => {
    try {
      const response = await axios.post("http://127.0.0.1:5500/predict", formData);
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 bg-white shadow-lg rounded-lg border border-gray-200 mt-8">
      <h2 className="text-2xl font-bold text-gray-800 mb-6 text-center">Budgeting Insights</h2>
      <div className="grid grid-cols-2 gap-4">
        {Object.keys(formData).map((key) => (
          key === "occupation" || key === "city_tier" ? (
            <div key={key} className="mb-4">
              <label className="block text-gray-700 font-medium capitalize" htmlFor={key}>{key.replace(/_/g, " ")}:</label>
              <select
                id={key}
              ...
              >
                {key === "occupation" && ["Self_Employed", "Retired", "Student", "Professional"].map(option => (
                  <option key={option} value={option}>{option}</option>
                ))}
                {key === "city_tier" && ["Tier_1", "Tier_2", "Tier_3"].map(option => (
                  <option key={option} value={option}>{option}</option>
                ))}
              </select>
            </div>
            ...
            <ul className="list-disc pl-6 mt-2">
              {suggestions.map((suggestion, index) => (
                <li key={index} className="text-gray-600">
                  <strong>{suggestion.category}:</strong> Reduce by ₹{suggestion.reduced_amount.toFixed(2)}. New amount: ₹{suggestion.new_amount.toFixed(2)}.
                </li>
              ))}
            </ul>
          )}
